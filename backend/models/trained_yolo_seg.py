...
CLASS_COLORS = generate_class_colors(len(CUSTOM_CLASSES))
...
def run_trained_yolo_seg(image_path: str):
    """
    YOLOv12x-seg pipeline:
      ...
      ✔ Sticker labels returned to frontend
    """
    ...
    sticker_paths = []
    sticker_labels = []   # ⭐ NEW → hold class names for each sticker
    ...
    for i, mask in enumerate(masks):
        ...
        cls = class_ids[i]
        class_name = CUSTOM_CLASSES[cls] if 0 <= cls < len(
            CUSTOM_CLASSES) else "object"
        conf = float(confs[i])

        # ⭐ Save the label for frontend display
        sticker_labels.append(class_name)
        ...
        sticker_path = outputs_dir / f"{stem}trained_sticker_{i}.png"
        cv2.imwrite(str(sticker_path), crop)
        sticker_paths.append(f"/outputs/{sticker_path.name}")

    # save overlay
    overlay_path = outputs_dir / f"{stem}trained_overlay.png"
    cv2.imwrite(str(overlay_path), overlay)

    return {
        "message": "Trained YOLOv12 segmentation successful",
        "overlay": f"/outputs/{overlay_path.name}",
        "stickers": sticker_paths,
        "labels": sticker_labels,   # ⭐ RETURN LABELS TO FRONTEND
    }
